...
G4double Tm=2*cm;//moderator thickness(2 cm)
G4double Tr=2*cm;//reflector tickness(2 cm)
...
Struct Box(G4String name,G4String mat_name,G4ThreeVector size,G4ThreeVector pos,G4LogicalVolume* mother_logic,G4int a)
{
...
{
    if(a==0)
 s._physical =new G4PVPlacement(rotm,pos,s._logic,name+"_PHYSI",mother_logic,false,0,true); 
else if(a==1)
{
 s._physical =new G4PVPlacement(rotm_det,pos,s._logic,name+"_PHYSI",mother_logic,false,0,true); 
}
else
...
G4Material* create_material(G4String name)
{
    ...
    {
     G4NistManager* nist = G4NistManager::Instance();
     G4Material* _world_mat = nist->FindOrBuildMaterial("G4_C"/*G4_BORON_CARBIDE"*/);
     return _world_mat;
    }
     else if(name=="b")
    {
     G4NistManager* nist = G4NistManager::Instance();
     G4Material* _world_mat = nist->FindOrBuildMaterial("G4_B"/*E4_BORON_CARBIDE*/);
     return _world_mat;
    }
     else if(name=="water")
    {
     G4NistManager* nist = G4NistManager::Instance();
     G4Material* _world_mat = nist->FindOrBuildMaterial("G4_WATER"/*E4_BORON_CARBIDE*/);
     return _world_mat;
    }
     else if(name=="pyrex")
    {
     G4NistManager* nist = G4NistManager::Instance();
     G4Material* _world_mat = nist->FindOrBuildMaterial("G4_Pyrex_Glass"/*E4_BORON_CARBIDE*/);
     return _world_mat;
    }
    else if(name=="polyE_borated")
    {
//	    G4Material* B = new G4Material(name="Boron", 5, 10*g/mole, 1.03*g/cm3);
